    ...
    <nav className=" w-screen h-auto tablet:h-16 justify-between text-2xl bg-mainbg desktop:bg-transparent flex align-middle text-white min-w-full fixed left-0 bottom-0 border-t-2  tablet:border-t-0 tablet:top-0 tablet:border-b border-gray-600 z-30">
      ...
      <ul className="w-screen tablet:w-2/3 laptop:w-1/2 text-primary text-3xl laptop:text-3xl font-main grid grid-cols-5 tablet:grid-cols-4 border-0 py-3 px-0 m-0">
        <Link to="/" className="navLink tablet:hidden">
          ...
          <AiFillHome
            className={`${
              pathname === '/' ? 'inline-block text-secondary' : 'hidden'
            } tablet:hidden`}
          />
          <span className="hidden">Home</span>
        </Link>
        <Link to={`/about/Jan`} className="navLink">
          <MdPersonOutline
            className={`${
              pathname.includes('about') ? 'hidden' : 'inline-block'
            } tablet:hidden`}
          />
          <MdPerson
            className={`${
        ...
        <Link to={`/skills/Jan`} className="navLink">
          <VscTools
            className={`${
              pathname.includes('skills') ? 'text-secondary' : 'text-primary'
            } tablet:hidden`}
          />
          <span className="hidden tablet:inline-block">skills</span>
        </Link>
        <Link to={`/projects/Jan`} className="navLink">
          <VscFolderActive
            className={`${
              pathname.includes('projects') ? 'text-secondary' : 'text-primary'
            } tablet:hidden`}
          />
          <span className="hidden tablet:inline-block">projects</span>
        </Link>
        <Link to={`/contact/Jan`} className="navLink">
          <RiContactsBookLine
            className={`${
              pathname.includes('contact') ? 'hidden' : 'inline-block'
            } tablet:hidden`}
          />
          <RiContactsBookFill
            className={`${
